...
public class ZkWatch implements Watcher {
    ...
    public static final Logger LOG = LoggerFactory.getLogger(ZkWatch.class);
    ...
    public String readData( String path ){
        String data = null;
        try {
            data = new String( this.zk.getData( path, this, null ) );
    ...
    public List<String> getChild( String path ){
        try{
            List<String> list=this.zk.getChildren( path, this );
            if(list.isEmpty()){
                LOG.info( "中没有节点" + path );
            }
            return list;
    ...
    public static void main(String [] args) throws InterruptedException {
        ...
        ZkWatch zkWatchAPI = new ZkWatch();
        ...
        if(null != childPaths){
            for(String childPath : childPaths){
                LOG.info(" |--节点名[" +  childPath +  "]");
            }
        }

        // 判断节点是否存在
        LOG.info( "检测节点[" + rootPath + "]是否存在:" + zkWatchAPI.isExists(rootPath)  );
        LOG.info( "检测节点[" + child1Path + "]是否存在:" + zkWatchAPI.isExists(child1Path)  );
        LOG.info( "检测节点[" + child2Path + "]是否存在:" + zkWatchAPI.isExists(child2Path)  );
